...
**Date:** 2026-06-11
**Status:** ✅ **BUILT AND DEPLOYED 2026-09-02.** `/infusion-therapy.html` created as a prerendered top-level page (`infusion-therapy.jsx` + shell, registered in `build.mjs`), with MedicalTherapy and FAQPage JSON-LD, the on-site/no-referral differentiation framing from §2 and §5, and Boca Raton NAP. All 12 condition-page links repointed from the interim `/florida.html#infusion` to this page; the homepage 'Infusion Therapy Services' card now links here instead of `conditions.html` (via a new optional `href` on `SERVICES`); the legacy `/infusion-therapy` redirect now lands on the real page instead of `/`. **Two deviations from spec, both deliberate:** (1) canonical uses the `www` host, not the apex — the apex 307s to www as of `534754e`; (2) the title in the spec was 67 characters, over its own ≤60 limit, so it was shortened to `Biologic & IV Infusion Therapy in Boca Raton, FL` (47). **One open content question for Yonah:** the spec says infusions are administered in Boca Raton only, but the live site already states both offices offer on-site infusion (`about.html` FAQ, and `location.jsx` renders the infusion block for NY as well). This page follows the live site and says both, directing patients to call to confirm their medication. If Boca Raton is in fact the only infusion suite, the Queens FAQ answer here and the existing copy in `about.html` and `location.jsx` all need correcting together.
