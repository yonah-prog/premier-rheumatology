...
**Status:** ⚠️ **OPEN — SYMPTOM NOT REPRODUCING as of 2026-09-02.** Ten consecutive apex fetches were all served by Vercel (`server: Vercel`, 307 → `www`) with **zero** WordPress markers (`wp-content` / `wp-includes` / `hello-biz`). That satisfies the multiple-consecutive-healthy-fetches half of this WO's re-close criterion but **not** the second half: confirmation that the legacy WordPress host / DNS record is fully decommissioned remains **unverified** and needs human check of the DNS zone and the old host. Keep open until that is confirmed.
...
- **2026-06-12 morning deep pass — 🚨 apex served LEGACY WORDPRESS again.** Fetched `https://premierjointcare.com/` and got the legacy WP homepage: title "Premier Rheumatology | Florida's Top Joint & Autoimmune Care"; phone **(561) 440-3270** + Fax (561)-440 3280; `/wp-content/uploads/2025/10/Asset-1.png` + `/wp-content/uploads/2026/02/Dr-Muriel-Boucher-1.png`; lorem-ipsum "What is physiotherapy?" FAQ; **Boucher + Akerman only, no Dr. Bruce Stein / no Queens / no Coconut Creek**; footer "© 2026 Specialty infusions FL Inc"; nav to legacy WP routes. Repo source re-verified correct this pass (deploy/serving problem, not code): `index.html` title "Rheumatologist in Boca Raton, FL & Queens, NY | Premier Rheumatology", canonical → apex, phone 730-3894, 0 WP markers; `florida.html` Suite 230 ×5 + Suite 236 ×3, no "234"; `new-york.html` correct title/canonical + Dr. Bruce Stein ×2. `/florida` + `/new-york` not independently fetchable (the WP apex fetch kept the React location URLs out of the web_fetch provenance set). Two-origin ratio this week: clean 06-11 midday → WP 06-12 morning. Required human action unchanged. Recorded in `seo-daily-notes/2026-06-12.md`.
